...
router = APIRouter(prefix="/api", tags=["UserExcercises"])

# ====================== MANAGING - USER FOODS ====================== #


@router.post("/user-excercises/{user_id}", status_code=status.HTTP_201_CREATED)
def create_user_excercises(user_id: float, user_excercise_data: UserExcerciseCreationDTO, db: DBSessionDependency):
    UserExcercisesService(db).create_user_excercise(
        user_id, user_excercise_data)
